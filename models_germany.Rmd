...
sarima_germany <- data_elec %>%
  select(Date, Germany) %>%
  as.data.frame() %>%
  { auto.arima(ts(.$Germany, frequency = 12), seasonal = TRUE) }

# Forecasting using the models
forecast_sarima_germany <- forecast(sarima_germany, h = 13)


# Graphing the forecast
autoplot(forecast_sarima_germany, include = 50) +
  labs(title = "SARIMA Forecast for Germany") +
  xlab("Year") + ylab("Electricity Consumption") 
  

# Extracting the forecasted values
forecast_table_germany_sa <- data.frame(Model = c("SARIMA"),
                                     Germany_sarima = forecast_sarima_germany$mean)


# Get the accuracy scores
accuracy_sarima_germany <- accuracy(sarima_germany)

forecast_table_germany_sa
accuracy_sarima_germany
