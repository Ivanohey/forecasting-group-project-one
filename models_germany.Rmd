...
We can see strong seasonality and trend in German data from decomposition analysis, so we tested ARIMA, ETS, and Seasonal NAIVE models as we assume these models can handle these types of data the best. Below is the plot of how well each model fits the data based on a 60-period prediction starting from 2018. 
```{r}

#Split the data
training_ge <- data_elec %>% select(Date,Germany) %>%  filter(Date < as.Date("2017-01-01"))
test_ge <- data_elec %>% select(Date,Germany) %>% filter(Date >= as.Date("2017-01-01"))
...
all_forecasts_ge <- bind_rows(
  data.frame(Date = forecasts_ge_a$Date, Method = "ARIMA", Forecast = forecasts_a$.mean),
  data.frame(Date = forecasts_ge_e$Date, Method = "ETS", Forecast = forecasts_e$.mean),
  data.frame(Date = forecasts_ge_n$Date, Method = "SNAIVE", Forecast = forecasts_n$.mean))


# Plot all three sets of forecasts together
autoplot(data_elec, series = "Germany") +
