...
forecast_ets_austria <- ets_austria %>% forecast(h = 12)

#Graphing the forecast
ets_austria %>% forecast(h = 12) %>% autoplot(Austria_since_2015)+
  labs(title = "ETS ANA Forecast for Austria")

#Extracting the forecasted values
forecast_values_ets_austria <- forecast_ets_austria$mean
...
colnames(forecast_table_austria)[1] <- "Italy_ets_ANA"
