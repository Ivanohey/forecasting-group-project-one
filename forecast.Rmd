...
For each of our country forecasts, we have decided to use the model with the lowest RMSE as shown in the previous section. This is because we want our models to be as "accurate" as possible, in away that we minimize the error/distance between our forecast values and the actual values. In summary, we used SARIMA for Italy, SNaive for Lichtenstein and ETS for all other countries.

#### Side note: 

Below, for each country, you can see a residual analysis for the every selected model, a graph of the forecast, and finally the forecast table. The outcome of the residual analysis is to confirm that the model has done a good job fitting the data, and more specifically, here's what we obtained from it: 
...
Further Analysis require introducing covariates, also known as predictors. They capture patterns and external factors that impact the forecasted variable. Adding covariates into forecasting models allows for a better understanding of the underlying dynamics and improves the overall accuracy of predictions.

For instance, in the case of Lichtenstein the SNaive model gives relatively good performance, despite its simplicity. However, to further enhance the forecast, it is essential to consider some covariate which might impact the trend of electricity demand such as economical crisis, technological such as the implementation of wind power which might affect drastically the electricity demand. 

Another example is the case of Austria, it is possible to improve the accuracy and the overall model performance by adding a covariate. Indeed, Austria face a drastic increase during 2018-2021, thus covariate such as weather conditions and economical activities would be interesting to incorporate. 